...
my ($infile , $min_num , $uniq_num, $germline_len , $name,$min_num_r , $uniq_num_r);
...
for my $pos(sort {$a<=>$b} keys %freq)
{
	my $flag = 0;
	for (keys %{$freq{$pos}}){
		if($freq{$pos}{$_} >= $min_num_r*$abund_all && $freq{$pos}{$_} >= $min_num){
			$flag = 1;
		}else{
			delete $freq{$pos}{$_};
		}
...
#		last if(scalar keys %add_bases_h == 0);
	...
	for(keys %add_bases_h){
		...
		if($uniq_n_2{$_}>=$uniq_num_r*$uniq_all_new && $uniq_n_2{$_}>=$uniq_num && $abund_2{$_} >= $min_num_r*$abund_all_new && $abund_2{$_} >= $min_num){
			push @success,$_;
			$flag = 1;
		}
	}
